  ...
  try {
    ...
    if (Array.isArray(advisors) && advisors.length > 0) {
      ...
      if (typeof bestAdvisor.advisor != "undefined" && bestAdvisor.score > 0) {
        ...
        }; //note answers.matchedAdvisor stores object {uuid:xxx, score:xxx}
    ...
    if (x == "postgradProgramTitle" || x == "undergradProgramTitle") {
      ...
      console.log(
        "Criteria is: " +
          ...
          similarity +
          " & score is: " +
          similarity * criteria[x]
      );
    } else if (x == "postgradProgramTitlePrev" && advisor[x]) {
      let similarity = stringSimilarity.compareTwoStrings(
        applicant["postgradProgramTitle"],
        advisor[x]
      );
      totalScore += similarity * criteria[x];
      console.log(
      ...
      x == "postgradUniPrev" &&
      advisor[x] &&
      applicant["postgradUni"] == advisor[x]
    ) {
      totalScore += criteria[x];
      console.log("Criteria is: " + x + " & score is: " + criteria[x]); //DEBUG ONLY
    } else if (
      x == "postgradFieldPrev" &&
      advisor[x] &&
      applicant["postgradField"] == advisor[x]
    ) {
      totalScore += criteria[x];
      console.log("Criteria is: " + x + " & score is: " + criteria[x]); //DEBUG ONLY
    } else if (
      x == "postgradTypePrev" &&
      advisor[x] &&
      applicant["postgradType"] == advisor[x]
    ) {
      totalScore += criteria[x];
      console.log("Criteria is: " + x + " & score is: " + criteria[x]); //DEBUG ONLY
